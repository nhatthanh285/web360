...
var vm = new Vue({
    ...
    methods: {
        submit: function() {
            ...
            this.sendData({url: actionUrl, method: this.method, data: this.row})
                .then(this.success, this.failed);
        },
        getData: function () {
            this.sendData({url: this.url.show + this.row.id, method: 'GET'})
                .then(this.success, this.failed);
        },
        sendData: function(parameters) {
    ...
    events: {
        ...
        'vuetable:cell-dblclicked': function(item, field, event) {
            this.$editable(event, function(value){
                ...
                vm.sendData({url: vm.url.update + data.id, method: 'PATCH', data: data}).then(
                function (response) {
